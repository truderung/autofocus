...
def context():
  ...
  context = dict()
  context["y_images"] = y_images
  context["z_images"] = z_images
  context["mask"] = mask
...
def test_calibrators_construction(context):
  assert type(CalibrateY()) == CalibrateY
  assert type(CalibrateZ()) == CalibrateZ


def test_y_calibration_valid(context):
  data = context
  y_flag, y_correction, all_indexes = CalibrateY()(data['y_images'], data['mask'], SEG_HEIGHT, CROSSING_DISTANCE_H)
  assert y_flag == "ok"
  assert type(y_correction) == float
  assert len(all_indexes) != 0

def test_y_calibration_unvalid(context):
  data = context
  y_flag, y_correction, all_indexes = CalibrateY()(data['z_images'], data['mask'], SEG_HEIGHT, CROSSING_DISTANCE_H)
  assert y_flag == "error"
...
# test_y_calib(y_images, test_mask)


# _, test_y_correction, _ = CalibrateY()(y_images, test_mask, SEG_HEIGHT, 770)
# test_z_images = []
# for p in IMG_STACK_Y_CALIB:
#   img = cv.imread(p.as_posix(), cv.IMREAD_GRAYSCALE)
